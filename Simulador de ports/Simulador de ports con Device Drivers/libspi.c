...
#if (HARDWARE == SERIAL || HARDWARE == PARA)
...
void initspi(void)
{
	...
	if (fp == NULL) /* siempre se debe hacer esta comprobación*/
	{
            ...
            DEBUG_PRINT((stdout, "Cannot open rfbb File. Try again later.\n"));
            sprintf(bufout,"no puedo abrit rfbb \n");
	n=strlen(bufout);
	write(fd, &bufout, n);
...
//  Write
	...
	// Print TTY //
	sprintf(bufout,"spi writee: %.2X \n",regData);
	n=strlen(bufout);
	write(fd, &bufout, n);
    // END TTY //
        
	DEBUG_PRINT((stdout, " Spi write:  %.2X \n"));

    
    
    kr_itoa(regData,buffstr);
	fprintf(fp,"%s",buffstr);
	fputc(0x00,fp);
...
void Set_Port_handles(void)
{
 ...
 /* kr_itoa:  convert n to characters in s */
 void kr_itoa(int n, char s[])
 {
     int i, sign;
